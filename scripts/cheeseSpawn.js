...
function spawnCheesesFromPolyline() {
    ...
    var usedIndices = {};
    var maxCheeseCount = Math.min(CHEESE_COUNT, mazePolylinePoints.length);

    while (cheeses.length < maxCheeseCount) {
        var randomIndex = Math.floor(Math.random() * mazePolylinePoints.length);
        ...
        var point = mazePolylinePoints[randomIndex];
...
function drawCheeses() {
    if (!canvasContext || cheeses.length === 0) {
        return;
    }

    canvasContext.fillStyle = "#ffd400";

    cheeses.forEach(function (cheese) {
        canvasContext.fillRect(cheese.x, cheese.y, cheese.width, cheese.height);
    });
}
