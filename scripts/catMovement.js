...
function drawCat() {
    ...
    var point = catPathPoints[catPointIndex];
    var scaledX = point.x * canvas.width / ORIGINAL_WIDTH;
    var scaledY = point.y * canvas.height / ORIGINAL_HEIGHT;
    var catLocationX = scaledX - CAT_SIZE / 2;
    var catLocationY = scaledY - CAT_SIZE / 2;
    ...
    catMoveFrameCounter++;
    if (catMoveFrameCounter >= catMovePause) {
        catMoveFrameCounter = 0;
        catPointIndex = (catPointIndex + 1) % catPathPoints.length;
    }

    if (Math.abs(playerX - catLocationX) <= 10 &&
        Math.abs(playerY - catLocationY) <= 10) {
        ripRat();
    }
}
